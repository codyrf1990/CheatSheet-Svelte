...
This repo has two live production branches, each with its own domain on Vercel:

| Branch               | Purpose                  | Status       |
| -------------------- | ------------------------ | ------------ |
| `main`               | **v1 — production site** | Live, stable |
| `feature/build-mode` | **v2 — production site** | Live, active |

**CRITICAL RULES:**

- **NEVER push to `main`** unless explicitly told to. v1 is frozen/stable.
- **NEVER merge `feature/build-mode` into `main`** — they are separate products with separate domains.
- All new work goes on `feature/build-mode` (or branches off it).
- If you need a working branch, branch from `feature/build-mode`, not `main`.
- PRs should target `feature/build-mode` unless the user says otherwise.
...
6. **Branches:** Never touch `main` — all work targets `feature/build-mode` (see above)
